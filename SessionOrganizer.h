...
public:
    ...
    void printBestSessionOrganiser(char *);

    void newRandomState ( );
    void changeBest();

    double printGoodness();
    double printBestGoodness();
    double getProcessingTimeInMinutes ( );

    double newState();
    double calculateNewCost(int tracka, int sessiona, int papera, int ida, int trackb, int sessionb, int paperb, int idb);
    void printIt();
